...
class Lib_wechat extends Library {
    ...
    public function get_oauth_code($appid,$query_string = null) {
        if (!isset($_GET['code'])) {
            $self_url = 'http://' . $_SERVER['HTTP_HOST'] .'/main/routes' . '?' . $_SERVER['QUERY_STRING'].$query_string;
//            die($self_url);
...
//    }
     ...
     */
    public function get_open_id($appid, $appsecret,$query_string = null) {

        if (!isset($_GET['code'])) {
            $this->get_oauth_code($appid,$query_string);
        }
